...
template<short nr0, typename args_t>
void kernel_mul_mv_q2_b3_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_q2_b3 * ax[nr0];
    for (int row = 0; row < nr0; ++row) {
        const uint64_t offset0 = (first_row + row)*args.nb01 + (i12/FC_mul_mv_r2)*args.nb02 + (i13/FC_mul_mv_r3)*args.nb03;
        ax[row] = (device const block_q2_b3 *) ((device char *) src0 + offset0);
    }
    ...
    // 32 lanes per simdgroup; each lane decodes 4 of the 128 trits per block,
    // then simd_sum reduces the lanes to the full 128-wide dot per row.
    for (int ib = 0; ib < nb; ++ib) {
        device const float * yb = y + ib*QK2_B3;
        for (short row = 0; row < nr0; ++row) {
            device const block_q2_b3 & xb = ax[row][ib];
            float sum = 0.f;
            for (short e = 0; e < 4; ++e) {
                const int j = tiisg + N_SIMDWIDTH*e;
                const int c = j >> 5, t = j & 31;
                const int byte  = (t < 30) ? (6*c + t/5) : (24 + (c >> 1));
                const int digit = (t < 30) ? (t % 5)     : (2*(c & 1) + (t - 30));
                const int q = ((int) xb.qs[byte] / pw3[digit]) % 3;
                sum += (float) (q - 1) * yb[j];
            }
            sumf[row] += xb.d * sum;
        }
...
[[host_name("kernel_mul_mv_q2_b3_f32")]]
kernel void kernel_mul_mv_q2_b3_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    kernel_mul_mv_q2_b3_f32_impl<N_R0_Q2_B3, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, nullptr, tgpig, tiisg, sgitg);
}
...
template [[host_name("kernel_mul_mv_id_q2_b3_f32")]]   kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_q2_b3_f32_impl <N_R0_Q2_B3>>>;
